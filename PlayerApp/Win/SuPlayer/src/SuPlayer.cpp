...
* SuPlayer.cpp
*
* This file for SuPlayer win32 application implementation.
* A win32 App define, focus UI and User Interface interaction call.
*
        ...
        case WM_COMMAND:
        {
            ...
            switch (wID)
            {
                case IDM_FILE_OPEN:
                {
                    ...
                    ofn.lpstrFilter = TEXT("媒体文件\0*.3gp;*.mpg;*.mpeg;*.avi;*.rmvb;*.rm;*.ra;*.ram;*.mp3;*.mid;*.wav;*.mov;*.qt;*.wmv;*.wma;*.asf;*.vob;*.dat;*.mkv;*.mp4;*.mka;*.midi;*.ogg;*.omg\0所有文件(*.*)\0*.*\0\0");
                ...
                case IDM_SIZE_50:
                {
                    UpdateMainWindow(g_MediaInfo.nWidth, g_MediaInfo.nHeight,0.5);
                }break;
                case IDM_SIZE_75:
                {
                    UpdateMainWindow(g_MediaInfo.nWidth, g_MediaInfo.nHeight, 0.75);
                }break;
                case IDM_SIZE_100:
                {
                    UpdateMainWindow(g_MediaInfo.nWidth, g_MediaInfo.nHeight, 1);
                }break;
                case IDM_SIZE_150:
                {
                    UpdateMainWindow(g_MediaInfo.nWidth, g_MediaInfo.nHeight, 1.5);
                }break;
                case IDM_SIZE_FULL:
                {
                ...
                case IDM_ABOUT:
                {
                    MessageBox(g_hWnd, TEXT(" 速播(SuPlayer): Baseed on FFmpeg.\r\n A clean and easy-to-use media player.\r\n E-mail:  epengao@126.com. \r\n QQ:  380154185"), TEXT("关于"), MB_ICONINFORMATION);
                }break;
            }
        } /* case WM_COMMAND: */
        break;
...
{
    _tcscpy(g_StatusBarText1, TEXT("已停止"));
    SetWindowText(g_hWnd, TEXT("SuPlayer"));
    g_hIcon = NULL;
...
bool PlayFile(const TCHAR* pszFileName)
{
    ...
    if (g_pMediaEng == NULL)
    {
        ...
        notify.NotifyHandler = MediaoNotifyHandler;
    ...
    SetTimer(g_hWnd, TIMER_GETPRGRESS, 250, NULL);
    ...
    UpdateMainWindow(g_MediaInfo.nWidth, g_MediaInfo.nHeight, 1.0);
    ShowWindow(g_hWndVideoScreen, SW_SHOW);
...
int MediaoNotifyHandler(void* pUserData, int ID, void* pParam1, void* pParam2)
{
    switch (ID)
    {
        case MediaEngineNotify_MediaStoped:
        {
            DebugPrintf("======MediaStoped===========\n");
            break;
        }
        case MediaEngineNotify_SeekCompleted:
        {
            DebugPrintf("======SeekCompleted===========\n");
            g_bIsSeeking = false;
            break;
        }
        case MediaEngineNotify_PlaybackFinished:
        {
            Close();
            break;
        }
